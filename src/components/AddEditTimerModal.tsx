...
export const AddEditTimerModal: React.FC<AddEditTimerModalProps> = ({
  ...
  const [hours, setHours] = useState(Math.floor(timer.duration / 3600) || 0);
  const [minutes, setMinutes] = useState(
    Math.floor((timer.duration % 3600) / 60) || 0
  );
  const [seconds, setSeconds] = useState(timer.duration % 60 || 0);
